...
fn commit_benchmark(c: &mut Criterion) {
  for num_vars in (10..=24).step_by(2) {
    ...
    group.bench_function(&name, move |b| {
      b.iter(|| {
        SamaritanMLPCS_Bls12_381::commit_G1(&srs, &mlp).unwrap();
      });
    });
    group.finish();
  }
...
fn eval_prove_benchmark(c: &mut Criterion) {
  for num_vars in (10..=24).step_by(2) {
    ...
    group.bench_function(&name, move |b| {
      b.iter(|| {
        SamaritanMLPCS_Bls12_381::prove(&srs, &mlp, &point, eval).unwrap();
      });
    });
    group.finish();
  }
...
fn eval_verify_benchmark(c: &mut Criterion) {
  for num_vars in (10..=24).step_by(2) {
    ...
    let eval_proof = SamaritanMLPCS_Bls12_381::prove(&srs, &mlp, &point, eval).unwrap();

    let name = format!("Samaritan_MLPCS_eval_verify_{}", num_vars);

    group.bench_function(&name, move |b| {
      b.iter(|| {
        SamaritanMLPCS_Bls12_381::verify(&srs, &comm, &point, eval, &eval_proof).unwrap();
      });
    });
    group.finish();
  }
